...
class SoundVoltexNabla(
    EventLogHandler,
    SoundVoltexBase,
):
    name: str = 'SOUND VOLTEX VII NABLA'
    version: int = VersionConstants.SDVX_NABLA
    ...
    GAME_CLEAR_TYPE_MAXXIVE_CLEAR: Final[int] = 4
    GAME_CLEAR_TYPE_ULTIMATE_CHAIN: Final[int] = 5
    GAME_CLEAR_TYPE_PERFECT_ULTIMATE_CHAIN: Final[int] = 6
    ...
    def handle_game_sv7_exception_request(self, requset: Node) -> Node:
        return Node.void('game')

    def handle_game_sv7_lounge_request(self, requset: Node) -> Node:
        game = Node.void('game')
        # Refresh interval in seconds.
        game.add_child(Node.u32('interval', 10))
        return game

    def handle_game_sv7_entry_s_request(self, request: Node) -> Node:
        game = Node.void('game')
        # This should be created on the fly for a lobby that we're in.
        game.add_child(Node.u32('entry_id', 1))
        return game

    def handle_game_sv7_entry_e_request(self, request: Node) -> Node:
    ...
    def __get_apigene(self) -> Dict[str, Any]:
        return {
            'info': [
                {
                    'apigene_id': 1,
            ...
            'catalog': [
                {
                    ...
                    'items': [
                        {
                            'type': 23,
                            'item_ids': [i + 1 for i in range(0, 26)]

                        },
                        {
                            'type': 24,
                            'item_ids': [i + 1 for i in range(0, 136)]
                        }
                    ]
                }
            ]
    ...
    def handle_game_sv7_common_request(self, request: Node) -> Node:
        game = Node.void('game')
        ...
        valgene = Node.void('valgene')
        game.add_child(valgene)

        apigene = Node.void('apigene')
        game.add_child(apigene)

        apigene_data = self.__get_apigene()
        for info in apigene_data['info']:
            inf = Node.void('info')
            apigene.add_child(inf)
            inf.add_child(Node.s32('apigene_id', info['apigene_id']))
            inf.add_child(Node.string('name', info['name']))
            inf.add_child(Node.string('name_english', info['name_english']))
            inf.add_child(Node.s32('common_rate', info['common_rate']))
            inf.add_child(Node.s32('uncommon_rate', info['uncommon_rate']))
            inf.add_child(Node.s32('rare_rate', info['rare_rate']))
            inf.add_child(Node.s32('price', info['price']))
            inf.add_child(Node.bool('no_duplicate', info['no_duplicate']))

        for cata in apigene_data['catalog']:
            for item in cata['items']:
                for iid in item['item_ids']:
    ...
    def handle_game_sv7_shop_request(self, request: Node) -> Node:
    ...
    def handle_game_sv7_hiscore_request(self, request: Node) -> Node:
    ...
    def handle_game_sv7_load_request(self, request: Node) -> Node:
    ...
    def handle_game_sv7_frozen_request(self, request: Node) -> Node:
        game = Node.void('game')
        game.add_child(Node.u8('result', 0))
        return game

    def handle_game_sv7_new_request(self, request: Node) -> Node:
        refid = request.child_value('refid')
    ...
    def handle_game_sv7_load_m_request(self, request: Node) -> Node:
        refid = request.child_value('refid')
    ...
    def handle_game_sv7_load_r_request(self, request: Node) -> Node:
        refid = request.child_value('refid')
        game = Node.void('game')
    ...
    def handle_game_sv7_save_request(self, request: Node):
        refid = request.child_value('refid')
    ...
    def handle_game_sv7_save_m_request(self, request: Node) -> Node:
        clear_map = {
        ...
        stats = {
            'volforce': track.child_value('volforce'),
    ...
    def handle_game_sv7_save_e_request(self, request: Node) -> Node:
        # This has to do with Policy floor infection, but we don't
        # implement multi-game support so meh.
        game = Node.void('game')
        return game

    def handle_game_sv7_play_s_request(self, request: Node) -> Node:
        root = Node.void('game')
        root.add_child(Node.u32('play_id', 1))
        return root

    def handle_game_sv7_buy_request(self, request: Node) -> Node:
        refid = request.child_value('refid')
    ...
    def handle_game_sv7_save_c_request(self, request: Node) -> Node:
        refid = request.child_value('refid')
    ...
    def handle_game_sv7_save_mega_request(self, requset: Node) -> Node:
        return Node.void('game')

    def handle_game_sv7_print_request(self, request: Node) -> Node:
        game = Node.void('game')
